...
def run_WCGGA(knapsack, pop_size, crossover_probability, mutation_probability, max_time, max_generations, elite_size, initialization_feasability, random_seed):
    ITEMS_LENGTH = len(knapsack)
    POPULATION_SIZE = pop_size
    P_CROSSOVER = crossover_probability
    P_MUTATION = mutation_probability
    MAX_GENERATIONS = max_generations  # Safety stoppage
    MAX_TIME_S = max_time
    HALL_OF_FAME_SIZE = 10
    ELITE_SIZE = elite_size  # 1 Elite per group. Total 8 Elites.
    # Random Seed
    ...
    for gen in range(MAX_GENERATIONS):
        ...
        for father_population_index in range(len(populations)):
            ...
            ideal_partner_index = Population_Patromiety_Dict[
                ideal_partner_warlpiri.SemiPatrimoiety]  # Convert Warlpiri -> Population Index
